...
def huber(u,kappa):
    return torch.where(torch.abs(u)<=kappa,
                       0.5*u**2,
...
class QuantileLoss(nn.Module):
    ...
    def forward(self,yhat,y,tau,alpha=0.1,kappa=0.5):
        diff = yhat-y
        mask = (diff.ge(0).float()-tau).detach()
        return (mask*diff).mean()

def qLoss(Ypred,Ytrue,kappa=0.5,alpha=0.1):
    tau = Ypred[:,-1:]
    Yp = Ypred[:,:-1]
    err = Ytrue - Yp
    loss = torch.mean(torch.maximum(tau*err,(tau-1)*err))
    #loss = torch.mean(torch.abs(tau-torch.where(err<0,1,0))*huber(err,kappa)/kappa)
    return loss

def train2(model,Xtrain,Ytrain,n_epoch=1000,lr=1e-4,lr_f=1e-4,check=100,name='nothing',alpha=0.1,patience=50,shift=0.0,scale=1):
    Xtrain = torch.tensor(Xtrain).to(device)
    Ytrain = torch.tensor(Ytrain).to(device)
    optimizer = optim.Adam(model.parameters(),lr=lr)
    GAMMA = (lr_f/lr)**(1/n_epoch)
    ...
    for i in range(n_epoch):
        optimizer.zero_grad()
        epochs_no_improve += 1
        qs = scale*torch.rand((Xtrain.shape[0],1)).to(device)-shift
        Ypred = model(torch.cat((Xtrain,qs),dim=1))
        loss = lossfn(Ypred,Ytrain,(qs+shift)/scale)
    ...
    del Xtrain, Ytrain, optimizer, lossfn, losses, min_loss, epochs_no_improve
    torch.cuda.empty_cache()

def train_gen2(model,X,Y,bs=10000,n_epoch=1000,lr=1e-4,lr_f=1e-4,check=100,name='nothing',alpha=0.1,patience=50,shift=0.0,scale=1):
    losses = []
    ...
    lossfn = QuantileLoss()
    optimizer = optim.Adam(model.parameters(),lr=lr)
    GAMMA = (lr_f/lr)**(1/(n_epoch*(X.shape[0]//bs)))
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=GAMMA)
    for i in range(n_epoch):
        ...
        for perm in perms:
            ...
            optimizer.step()
            scheduler.step()
            del Ypred,loss,qs,Xtrain,Ytrain
            torch.cuda.empty_cache()
        epochs_no_improve += 1
        if (i+1)%check == 0:
            print("Epoch {}, Loss: {}".format(i+1,losses[-1]))
    ...
    del X,Y
    torch.cuda.empty_cache()
    plt.plot(np.arange(len(losses))/nbatch,losses)
...
def correctAll_v2(Y,x,qbkg,qdata,Yout):
    N,M = qbkg.shape
    bad = 0
    bad_bkg = 0
    bad_data = 0
...
class chainedTrainer:
    ...
    def trainQuantiles(self,trainDict,i,nTrain,corrDict=None,lr=1e-3,n_epoch=1000,bs=10000,batched=False,q_per=10):
        ...
        model = qreg(Xtrain.shape[1]+1,widths=self.widths[i-1],
    ...
    def trainBkg(self,nTrain=200000,iMin=1,iMax=None,lr=1e-3,n_epoch=1000,bs=10000,batched=False,correctFirst=False,q_per=10):
        ...
        for i in range(iMin,endRange):
            ...
            # Prepare tensors
            Xeval = np.concatenate([self.corrBkg_dict[self.varDict[v]].reshape(-1,1) for v in self.varOrder[:i]],axis=1)
            ...
            dataQuantiles = []
            for q in quantiles:
                Xin = torch.tensor(np.concatenate((Xeval,q*np.ones((Xeval.shape[0],1))),axis=1),dtype=torch.float32).to(device)
                ybkg = model(Xin).detach().cpu().numpy()[:,:-1]
                bkgQuantiles.append(ybkg)
                ydata = data_model(Xin).detach().cpu().numpy()[:,:-1]
                dataQuantiles.append(ydata)
                del Xin
                torch.cuda.empty_cache()
            bkgQuantiles = np.concatenate(bkgQuantiles,axis=1)
            dataQuantiles = np.concatenate(dataQuantiles,axis=1)
