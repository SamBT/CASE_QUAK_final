...
class EdgeConvBlock(nn.Module):
    ...
    def __init__(self, k, in_feat, out_feats, batch_norm=True, activation=True, cpu_mode=False):
        ...
        for i in range(self.num_layers):
            self.bns.append(nn.BatchNorm2d(out_feats[i]) if batch_norm else None)

        self.acts = nn.ModuleList()
        for i in range(self.num_layers):
            self.acts.append(nn.ReLU() if activation else None)
...
class PNetWrapper(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        self.pn = ParticleNet(input_dims,num_classes,conv_params,fc_params,use_fusion=use_fusion,use_fts_bn=use_fts_bn,use_counts=use_counts,for_inference=for_inference,for_segmentation=for_segmentation,**kwargs)
    ...
    def forward(self,x):
        points = x[:,:,:2]
        features = x[:,:,:]
        out = self.pn(torch.permute(points,(0,2,1)),torch.permute(features,(0,2,1)))
        if self.act_out:
