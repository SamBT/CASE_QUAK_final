...
class QuakSpaceMaker:
    ...
    def run(self):
        ...
        perm = np.random.permutation(bkg_mjj.shape[0]+n_sig)
        labels = np.concatenate((np.zeros(bkg_mjj.shape[0]),np.ones(n_sig)),axis=0)[perm]
        if "BKG" in self.bkgLossFile:
            samp_labels = np.concatenate((bkg_label,np.ones(n_sig)),axis=0)[perm]
        else:
            samp_labels = np.concatenate((np.ones(bkg_mjj.shape[0]),np.ones(n_sig)),axis=0)[perm]
        j1_masses = np.concatenate((bkg_mj1,np.zeros(n_sig)),axis=0)[perm]
        j2_masses = np.concatenate((bkg_mj2,np.zeros(n_sig)),axis=0)[perm]
        j1_btag = np.concatenate((bkg_btag1,np.zeros(n_sig)),axis=0)[perm]
        j2_btag = np.concatenate((bkg_btag2,np.zeros(n_sig)),axis=0)[perm]
        evt_idx = np.concatenate((bkg_evtIdx,-1*np.ones(n_sig)),axis=0)[perm]
        ...
        nnScores_save = {}
        for k in nn_toSave:
            nnScores_save[k] = np.concatenate((bkg_nnscores[k],inj_nnscores[k]),axis=0)[perm]
