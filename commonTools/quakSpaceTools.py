...
def putVariable(fout,bname,data,singleton=False):
    if singleton:
        if bname in fout.keys():
            del fout[bname]
        fout.create_dataset(bname,data=data)
    else:
        if bname in fout.keys():
            prev_size = fout[bname].shape[0]
            new_size = data.shape[0] + prev_size
            fout[bname].resize(new_size,axis=0)
            fout[bname][prev_size:] = data
        else:
            if len(data.shape)==2:
                fout.create_dataset(bname,data=data,chunks=True,maxshape=(None,data.shape[1]))
            else:
                fout.create_dataset(bname,data=data,chunks=True,maxshape=(None,))
    ...
    def evaluateLosses(self,toEval,chunkSize=100000):
        ...
        if isSignal:
            ...
            if not os.path.exists(f"lossEvals/{fname}.h5") and not self.overwrite:
                ...
                with h5py.File(f"lossEvals/{fname}.h5","a") as fin:
                    existing_keys = list(fin.keys())
                    if self.modelsToEval != []:
                        for m in self.modelsToEval:
                            if m in existing_keys:
            ...
            for j,file in enumerate(tqdm(files)):
                ...
                with h5py.File(file,"r") as inFile:
                    ...
                    if isSignal:
                        ...
                        if 'lund_weights' in list(inFile.keys()):
                            writeLund = True
                            lund_weights = inFile['lund_weights'][()][cut]
                            lund_weights_stat_var = inFile['lund_weights_stat_var'][()][cut]
                            lund_weights_pt_var = inFile['lund_weights_pt_var'][()][cut]
                            lund_weights_sys_var = inFile['lund_weights_sys_var'][()][cut]
                            lund_weights_matching_unc = inFile['lund_weights_matching_unc'][()]
                ## write to output
                with h5py.File(f"lossEvals/{outName}.h5","a") as fout:
                    ...
                    if isSignal:
                        putVariable(fout,"sys_weights",sys_weights)
                        if writeLund:
                            putVariable(fout,"lund_weights",lund_weights)
                            putVariable(fout,"lund_weights_stat_var",lund_weights_stat_var)
                            putVariable(fout,"lund_weights_pt_var",lund_weights_pt_var)
                            putVariable(fout,"lund_weights_sys_var",lund_weights_sys_var)
                            putVariable(fout,"lund_weights_matching_unc",lund_weights_matching_unc,singleton=True)
        
        # loop over files
        for j,file in enumerate(files):
            print(f"file {j+1}/{len(files)}")
            ...
            for i,mod in enumerate(tqdm(models)):
                ...
                for varset, sysname in zip(variableSets,variableSetNames):
                    ...
                    for chunk in data:
                        ...
                        with torch.no_grad():
                            if conditional:
                                losses.append(-model.log_prob(inputs=chunk_tens[:,:-len(context_vars)],context=chunk_tens[:,-len(context_vars):]).detach().cpu().numpy())
                            else:
                                losses.append(-model.log_prob(chunk_tens).detach().cpu().numpy())
                        del chunk_tens
...
def normalizeLosses(decorr_type,doBoxCox=True,overwrite=True,toEval=[],files_to_eval=[]):
    if decorr_type == "pca":
    ...
    if files_to_eval == []:
        ...
        sig_files = ["lossEvals/"+f for f in files_to_eval]
    ...
    if files_to_eval == []:
        ...
        sig_files = [f"{outDir}/"+f for f in files_to_eval]
...
def lossPlots(bkg_axis,sig_axes,sig_alias,sample,reducs=[],decorr=True,base="lossEvals_pcaDecorr_boxCox"):
    from sklearn.linear_model import LinearRegression
    if decorr:
