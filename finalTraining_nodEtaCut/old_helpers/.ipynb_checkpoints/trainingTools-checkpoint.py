...
class NFTrainer:
    def __init__(self,sample,mean_reference,variables,nTrain=100000,NF_kwargs={},dEta_cut=-1,sideband=False,modelName=None,rangeScale=3,minmax=False):
        self.sample = sample
    ...
    def loadData(self,nevts,mode="train"):
        ...
        if type(self.sample) == list:
            ...
            data = hf.loadData(self.sample,self.variables,nMax=nevts,mode=mode,dEta_cut=self.dEta_cut,sideband=self.sideband)
...
def train(flow,trainData,NF_kwargs,bs=10000,n_epoch=100,patience=20,learning_rate=1e-3,print_interval=10,T_max=-1,eta_min=-1,scheduler=None,clip=None,outName="flow"):
    print(f"{len(trainData)} training events")
    if clip is not None:
        trainData = trainData[np.all(np.abs(trainData)<clip,axis=1)]
    if bs < 1:
    ...
    with open(f"flow_on_losses/{outName}.json","w") as f:
    ...
    train_losses = []
    patience_count = 0
    saveName = f"flow_on_losses/{outName}.pt"
    for i in range(n_epoch):
        if patience_count == patience:
            break
        epoch_losses = []
    ...
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.savefig(f"flow_on_losses/{outName}-train.pdf")
