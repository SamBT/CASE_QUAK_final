...
class NFTrainer:
    def __init__(self,sample,mean_reference,variables,context_vars,modelName,nTrain=100000,NF_kwargs={},dEta_cut=-1,sideband=False,rangeScale=3,minmax=False):
        self.sample = sample
        ...
        self.context_features = len(context_vars)
        ...
        self.NF_kwargs['context_vars'] = self.context_vars
        self.NF_kwargs['context_features'] = self.context_features
    ...
    def loadMeans(self):
        ...
        with h5py.File(f"input_h5s/{self.mean_reference}/{ref_file}","r") as f:
            for v in self.variables+self.context_vars:
    ...
    def loadData(self,nevts,mode="train"):
        ...
        if type(self.sample) == list:
            datas = []
            nev_per = int(nevts/len(self.sample))
            for samp in self.sample:
                datas.append(hf.loadData(samp,self.variables+self.context_vars,nMax=nev_per,mode=mode,
                                         dEta_cut=self.dEta_cut,sideband=self.sideband))
                print(f"loading {samp}, {len(datas[-1])} events")
            data = np.concatenate(datas,axis=0)
            np.random.shuffle(data)
        else:
            data = hf.loadData(self.sample,self.variables+self.context_vars,nMax=nevts,mode=mode,dEta_cut=self.dEta_cut,sideband=self.sideband)
    ...
    def new_flow(self):
        return make_flow(len(self.variables),self.context_features,self.NF_kwargs)
    ...
    def train(self,bs=10000,n_epoch=100,patience=20,learning_rate=1e-3,print_interval=10,T_max=-1,eta_min=-1,scheduler=None,clip=None):
        ...
        if clip is not None:
            cut = np.all(np.abs(trainData[:,:-self.context_features])<clip,axis=1)
            trainData = trainData[cut]
        if bs < 1:
            bs = int(bs*trainData.shape[0])
        loadTensor = torch.tensor(trainData,device=device)
        loader = utils.DataLoader(loadTensor,batch_size=bs,shuffle=True,generator=torch.Generator(device='cuda'))
        ...
        for i in range(n_epoch):
            ...
            for batch_idx, x in enumerate(loader):
                optimizer.zero_grad()
                inp,cont = x[:,:-self.context_features], x[:,-self.context_features:]
                loss = -flow.log_prob(inputs=inp,context=cont)[0].mean()
                loss.backward()
                optimizer.step()
    ...
    def plotDensity(self,nSamp,log=False):
        ...
        data,context = data[:,:-self.context_features], data[:,-self.context_features:]
        flow = self.get_model()
        flow = flow.to(device)
        with torch.no_grad():
            samples = flow.sample(1,context=context).detach().cpu().numpy().reshape(data.shape[0],-1)
...
def loadMeans(reference,variables):
    means = []
    stds = []    
    ref_file = "meanStd.h5" # no dEta cut!
    with h5py.File(f"input_h5s/{reference}/{ref_file}","r") as f:
        for v in variables:
            means.append(f[v+"_mean"][()])
            stds.append(f[v+"_std"][()])
    means = np.array(means)
